...
// @dokkebi-version: 4
// worker/api/_dokkebi/_panel/[[path]].ts
// Cloudflare Pages Function — 관제 어드민 API (서브패스 캐치올)
// POST /api/_dokkebi/_panel/auth          → 비밀번호 인증 → 토큰 발급
// GET  /api/_dokkebi/_panel/api/overview  → 개요 통계
// GET  /api/_dokkebi/_panel/api/errors    → 에러 로그
// GET  /api/_dokkebi/_panel/api/security  → 보안 이벤트
// GET  /api/_dokkebi/_panel/api/adl/*     → 능동 방어(ADL) 상태/블랙리스트 (Phase 3-C)
// GET  /api/_dokkebi/_panel/api/security-info → 보안 구성/커버리지 (Phase 2-⑧)
// GET  /api/_dokkebi/_panel/api/requests  → 요청 기록
// POST /api/_dokkebi/_panel/api/errors/clear → 에러 로그 삭제

// Phase 1-③ · Phase 2-⑧ 빌드 시 embed 된 보안 구성 요약.
const _SECURITY_SUMMARY: any = __DOKKEBI_PH_SUMMARY__;
const _PANEL_IP_GUARD_ENABLED = __DOKKEBI_PH_PANEL_GUARD__;
...
async function q(db: D1Database, sql: string, params: unknown[] = []) {
  try { const r = await db.prepare(sql).bind(...params).all(); return {rows:(r.results||[]) as Record<string,unknown>[]}; }
  catch { return {rows:[]}; }
}
...
export const onRequest: PagesFunction<Env> = async (ctx) => {
  ...
  try {
  // ctx.params.path: [[path]] 캐치올이 캡처한 서브패스 (Cloudflare에서는 배열로 전달됨)
  ...
  if (!ipGate.ok) {
    if (_internalDb(env)) {
      ctx.waitUntil(_internalDb(env).prepare(`INSERT OR IGNORE INTO "_dokkebi_security" ("id","type","ip","path","detail") VALUES (?,?,?,?,?)`)
        .bind(crypto.randomUUID(), 'panel_ip_block', ipGate.ip, url.pathname, 'IP not in DOKKEBI_PANEL_ALLOWED_IPS').run().catch(() => {}));
    }
    return json({error:'접근이 허용되지 않은 IP 입니다.'}, 403);
  }

  // trailing slash(/api/_dokkebi/_panel/) → HTML 직접 서빙 (리다이렉트 루프 방지)
  if (subPath === '/' && request.method === 'GET') {
    if (!adminPassword) {
      return new Response(
        '<!DOCTYPE html><html><body style="font-family:monospace;padding:40px;background:#0d1117;color:#e6edf3"><h2>⚙️ 도깨비 관제 어드민</h2><p>Cloudflare Worker 시크릿을 설정하세요:</p><pre style="background:#161b22;padding:16px;border-radius:8px">wrangler secret put DOKKEBI_ADMIN_PASSWORD</pre></body></html>',
        { status: 503, headers: { 'Content-Type': 'text/html' } }
      );
    }
    return new Response(PANEL_HTML.replaceAll('__PROJECT__', '__DOKKEBI_PH_PROJECT__').replaceAll('__MODE__', 'serverless'), {
      headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' },
    });
  }

  if (!adminPassword) return json({error:'DOKKEBI_ADMIN_PASSWORD 미설정'}, 503);
  ...
  if (subPath === '/auth' && request.method === 'POST') {
    ...
    const body = await request.json<any>().catch(() => ({} as Record<string,unknown>));
  ...
  // API (토큰 필수) — subPath 기준 판별로 /api/_dokkebi/_panel 자체가 걸리지 않음
  if (subPath.startsWith('/api/')) {
    const authHeader = request.headers.get('Authorization') || '';
    const token = authHeader.startsWith('Bearer ') ? authHeader.slice(7) : '';
    if (!await verifyToken(token, adminPassword)) return json({error:'인증이 필요합니다.'}, 401);
    ...
    if (subPath === '/api/overview') {
      const [err,sec,req24,act,trend] = await Promise.all([
        ...
        q(db,`SELECT date(ts) as day,COUNT(*) as cnt FROM "_dokkebi_requests" WHERE ts>=datetime('now','-7 days') GROUP BY day ORDER BY day`),
      ]);
      return json({activeUsers:act.rows[0]?.cnt??0, errors24h:err.rows[0]?.cnt??0,
        security24h:sec.rows[0]?.cnt??0, requests24h:req24.rows[0]?.cnt??0, trend:trend.rows, mode:'serverless'});
    }
    if (subPath === '/api/errors/clear' && request.method==='POST') {
      await db.prepare(`DELETE FROM "_dokkebi_errors"`).run();
      return json({ok:true});
    }
    if (subPath === '/api/security/clear' && request.method==='POST') {
      await db.prepare(`DELETE FROM "_dokkebi_security"`).run();
      await db.prepare(`DELETE FROM "_dokkebi_requests"`).run();
      return json({ok:true});
    }
    if (subPath === '/api/requests/clear' && request.method==='POST') {
      await db.prepare(`DELETE FROM "_dokkebi_requests"`).run();
      return json({ok:true});
    }
    // Phase 2-⑧ 보안 커버리지 + 빌드 서명 정보 (실시간 DB 조회 불필요)
    if (subPath === '/api/security-info') {
      return json(_SECURITY_SUMMARY);
    }
    if (subPath === '/api/errors') {
      const lv = qs.get('level')||''; const search = qs.get('search')||'';
      const from = qs.get('from')||''; const to = qs.get('to')||'';
      const conds: string[] = []; const params2: any[] = [];
      if (lv) { conds.push('level = ?'); params2.push(lv); }
      if (search) { conds.push('(message LIKE ? OR path LIKE ?)'); params2.push('%'+search+'%', '%'+search+'%'); }
      if (from) { conds.push("ts >= ?"); params2.push(from); }
      if (to) { conds.push("ts <= ?"); params2.push(to); }
      const where = conds.length ? 'WHERE ' + conds.join(' AND ') : '';
      const [rows,total] = await Promise.all([
        q(db,`SELECT id,ts,source,level,message,stack,path,method FROM "_dokkebi_errors" ${where} ORDER BY ts DESC LIMIT ? OFFSET ?`,[...params2,limit,offset]),
        q(db,`SELECT COUNT(*) as cnt FROM "_dokkebi_errors" ${where}`,params2),
      ]);
      return json({rows:rows.rows, total:total.rows[0]?.cnt??0});
    }
    if (subPath === '/api/security') {
      const ip = qs.get('ip')||''; const type2 = qs.get('type')||'';
      const from = qs.get('from')||''; const to = qs.get('to')||'';
      ...
      if (type2) {
        if (type2 === 'request') secConds.push('1 = 0');
        else { secConds.push('type = ?'); secParams.push(type2); reqConds.push('1 = 0'); }
      }
    ...
    if (subPath === '/api/requests') {
      const ip = qs.get('ip')||'';
      const from = qs.get('from')||''; const to = qs.get('to')||'';
      const conds: string[] = []; const params2: any[] = [];
      if (ip) { conds.push('ip LIKE ?'); params2.push('%'+ip+'%'); }
      if (from) { conds.push("ts >= ?"); params2.push(from); }
      if (to) { conds.push("ts <= ?"); params2.push(to); }
      const where = conds.length ? 'WHERE ' + conds.join(' AND ') : '';
      const [rows,total] = await Promise.all([
        q(db,`SELECT id,ts,method,path,status,duration_ms,ip FROM "_dokkebi_requests" ${where} ORDER BY ts DESC LIMIT ? OFFSET ?`,[...params2,limit,offset]),
        q(db,`SELECT COUNT(*) as cnt FROM "_dokkebi_requests" ${where}`,params2),
      ]);
      return json({rows:rows.rows, total:total.rows[0]?.cnt??0});
    }
    // Phase 3-C — ADL 상태/블랙리스트/제어
    if (subPath === '/api/adl/status') {
      const now = Date.now();
      const [bl,sus,lr,byKind,d24,b24] = await Promise.all([
      ...
      return json({config:cfg, blacklistCount:bl.rows[0]?.cnt??0, suspicionCount:sus.rows[0]?.cnt??0, byKind:byKind.rows,
        lastRun, lastRunAgoSec: lastRun? Math.floor((now-lastRun)/1000) : null,
        detect24h:d24.rows[0]?.cnt??0, block24h:b24.rows[0]?.cnt??0});
    }
    if (subPath === '/api/adl/blacklist') {
      const now = Date.now();
      const [rows,total] = await Promise.all([
    ...
    if (subPath === '/api/adl/run' && request.method==='POST') {
      try {
        const now = Date.now();
        await db.batch([
        ...
        const cfgAdl = (_SECURITY_SUMMARY && _SECURITY_SUMMARY.activeDefense) || {};
        const adlMode = String(cfgAdl.mode || 'monitor').toLowerCase();
    ...
    if (subPath === '/api/adl/blacklist/clear' && request.method==='POST') {
      try { await db.prepare(`DELETE FROM "_dokkebi_blacklist"`).run(); } catch { /* */ }
      return json({ok:true});
    }
    if (subPath === '/api/adl/suspicion/clear' && request.method==='POST') {
      try { await db.prepare(`DELETE FROM "_dokkebi_adl_suspicion"`).run(); } catch { /* */ }
      return json({ok:true});
    }
    if (subPath === '/api/adl/blacklist/delete' && request.method==='POST') {
      try {
    ...
    if (subPath === '/api/adl/suspicion/delete' && request.method==='POST') {
      try {
        const body = await request.json();
        const key = String(body.key || '');
        const kind = String(body.kind || 'ip');
    ...
    if (subPath === '/api/adl/blacklist/add' && request.method==='POST') {
      try {
        const body = await request.json();
    ...
    if (subPath === '/api/ai/analyze' && request.method === 'POST') {
      ...
      for (const row of secData) {
        ...
        if (row.ip) { const ip=String(row.ip); ipCounts[ip]=(ipCounts[ip]||0)+1; }
      }
      ...
      const errCount = Number(errStats.rows[0]?.cnt||0);
      const blActive = Number(blCount.rows[0]?.cnt||0);
...
위험도(낮음/중간/높음), 주요 위협 패턴, 권장 조치를 포함하여 4~6문장으로 요약하세요.`;
        try {
          const aiRes: any = await aiBinding.run('@cf/meta/llama-3.1-8b-instruct', {
            messages: [{role:'user', content:prompt}],
            max_tokens: 600,
          });
          report = aiRes?.response || aiRes?.choices?.[0]?.message?.content || '';
          if (report) usedAI = true;
        } catch { /* Workers AI 실패 시 규칙 기반으로 폴백 */ }
      }
      if (!report) {
        ...
        if (threats.length===0) {
          report = `현재 특별한 위협이 감지되지 않습니다. 지난 1시간 요청 ${reqStat.total}건 중 오류 ${reqStat.errors}건, 보안 이벤트 ${secData.length}건이 기록되었으며 정상 운영 중입니다.${blActive>0?' 현재 '+blActive+'개 IP가 차단 중입니다.':''}`;
        } else {
          report = `⚠️ 주의가 필요한 보안 이슈가 감지되었습니다.\n\n${threats.map(t=>`• ${t}`).join('\n')}${blActive>0?'\n\n현재 '+blActive+'개 IP가 차단 중입니다.':''}`;
        }
      }
  ...
  return new Response('Not Found', {status:404});
    ...
    return json({error:'관제 패널 내부 오류', detail:String(e?.message || e || 'unknown')}, 500);
  }
};
...
<div class="tab" id="tab-errors">
  ...
  <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:end;margin-bottom:12px">
    ...
    <button class="btn" style="width:auto;padding:6px 14px;font-size:13px;background:var(--red)" onclick="clearErrors()">전체 삭제</button>
  </div>
...
<div class="tab" id="tab-security">
  <h3 style="margin-bottom:12px">보안 이벤트</h3>
  <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:end;margin-bottom:12px">
    ...
    <button class="btn" style="width:auto;padding:6px 14px;font-size:13px;background:var(--red)" onclick="clearSecurity()">전체 삭제</button>
  </div>
...
async function doLogin(){const pw=document.getElementById('pw').value;if(!pw)return;
  ...
  const txt=await r.text();let d={};try{d=JSON.parse(txt);}catch{d={error:'서버가 JSON이 아닌 응답을 반환했습니다. 관리자 시크릿/라우팅을 확인하세요.'};}
  if(!r.ok){document.getElementById('err').textContent=d.error||('로그인 실패 ('+r.status+')');return;}
...
function _secRiskMeta(type, detail){const t=String(type||'');const d=String(detail||'').toLowerCase();if(t==='request'||t==='db_audit')return{label:'정상',cls:'badge-info'};if(t==='query_fallback')return{label:'정상(auto)',cls:'badge-info'};if(t==='query_learned'||t==='tenant_injected'||t==='adl_detect'||t==='mutation_budget'||t==='rate_limit')return{label:'주의',cls:'badge-warn'};if(t==='authz_denied')return{label:d.includes('auth_required')?'주의':'공격의심',cls:d.includes('auth_required')?'badge-warn':'badge-danger'};if(t==='adl_block'||t==='hmac_fail'||t==='sql_inject'||t==='replay_attempt'||t==='query_not_registered'||t==='raw_sql_blocked'||t==='sql_blocked'||t==='tenant_policy_violation')return{label:'공격의심',cls:'badge-danger'};return{label:'주의',cls:'badge-warn'};}
...
async function clearErrors(){if(!confirm('에러 로그를 모두 삭제할까요?'))return;try{const r=await fetch(_BASE+'/api/errors/clear',{method:'POST',headers:{'Authorization':'Bearer '+_t}});if(!r.ok)throw new Error('clear failed');_errPage=0;loadE();loadOv();}catch(e){alert('에러 로그 삭제 실패: '+e.message);}}
async function clearSecurity(){if(!confirm('이벤트/요청 통합 로그를 모두 삭제할까요?'))return;try{const r=await fetch(_BASE+'/api/security/clear',{method:'POST',headers:{'Authorization':'Bearer '+_t}});if(!r.ok)throw new Error('clear failed');_secPage=0;loadS();loadOv();}catch(e){alert('이벤트 삭제 실패: '+e.message);}}
